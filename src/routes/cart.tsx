...
export const Route = createFileRoute("/cart")({
  head: () => ({ meta: [{ title: "Shopping Bag" }, { name: "robots", content: "noindex" }] }),
  ssr: false,
  component: CartPage,
});
        ...
        {items.length === 0 ? (
          ...
          <div className="grid lg:grid-cols-[1fr_380px] gap-10">
                ...
                <li key={`${i.id}-${i.size ?? ""}`} className="py-6 flex gap-5">
                  ...
                  <div className="flex-1 flex flex-col">
                    <div className="flex justify-between gap-4">
                      ...
                      <p className="font-medium">{currency(i.price * i.quantity)}</p>
                    </div>
            ...
            <aside className="border border-hairline p-7 h-fit lg:sticky lg:top-28 space-y-4">
              ...
              <div className="flex justify-between text-sm pt-3"><span className="text-ink-soft">Subtotal</span><span>{currency(subtotal)}</span></div>
